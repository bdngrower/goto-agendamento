...
const { verificarRateLimitLogin } = require("../../lib/rateLimiter");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    res.setHeader(
      "Set-Cookie",
      `goto_admin_session=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=28800${isProd ? "; Secure" : ""}`
    );
    ...
    console.error("Erro interno no login administrativo:", err.message);
    return res.status(500).json({ success: false, error: "Erro interno no servidor ao processar login." });
  }
};
